...
class TestGetButtonStateConfig:
    ...
    def test_state_overrides_press_commands(self):
        """Per-state press command arrays override button-level press."""
        btn = {
            "press": [{"type": "cc", "cc": 20, "value": 127}],
            "keytimes": 2,
            "states": [
                {"press": [{"type": "cc", "cc": 30, "value": 100}]},
                {"press": [{"type": "note", "note": 60, "velocity": 90}]}
            ]
        }
        # State 1 should have overridden press
        result1 = get_button_state_config(btn, 1)
        assert result1["press"] == [{"type": "cc", "cc": 30, "value": 100}]
        # State 2 should have overridden press
        result2 = get_button_state_config(btn, 2)
        assert result2["press"] == [{"type": "note", "note": 60, "velocity": 90}]

    def test_state_overrides_release_commands(self):
        """Per-state release command arrays override button-level release."""
        btn = {
            "release": [{"type": "cc", "cc": 20, "value": 0}],
            "keytimes": 2,
            "states": [
                {"release": [{"type": "cc", "cc": 30, "value": 10}]},
                {}  # State 2 has no release override
            ]
        }
        result1 = get_button_state_config(btn, 1)
        assert result1["release"] == [{"type": "cc", "cc": 30, "value": 10}]
        # State 2 should fall back to button-level release
        result2 = get_button_state_config(btn, 2)
        assert result2["release"] == [{"type": "cc", "cc": 20, "value": 0}]

    def test_state_overrides_long_press_commands(self):
        """Per-state long_press command arrays override button-level long_press."""
        btn = {
            "long_press": [{"type": "cc", "cc": 40, "value": 127}],
            "keytimes": 2,
            "states": [
                {"long_press": [{"type": "cc", "cc": 50, "value": 80}]},
                {"long_press": []}  # Empty array
            ]
        }
        result1 = get_button_state_config(btn, 1)
        assert result1["long_press"] == [{"type": "cc", "cc": 50, "value": 80}]
        # State 2 explicitly has empty long_press
        result2 = get_button_state_config(btn, 2)
        assert result2["long_press"] == []

    def test_state_overrides_long_release_commands(self):
        """Per-state long_release command arrays override button-level long_release."""
        btn = {
            "long_release": [{"type": "cc", "cc": 40, "value": 0}],
            "keytimes": 2,
            "states": [{"long_release": [{"type": "pc", "program": 5}]}]
        }
        result = get_button_state_config(btn, 1)
        assert result["long_release"] == [{"type": "pc", "program": 5}]

    def test_state_overrides_multiple_event_types(self):
        """State can override multiple event types simultaneously."""
        btn = {
            "press": [{"type": "cc", "cc": 20, "value": 127}],
            "release": [{"type": "cc", "cc": 20, "value": 0}],
            "keytimes": 2,
            "states": [
                {
                    "press": [{"type": "note", "note": 60, "velocity": 100}],
        ...
        result = get_button_state_config(btn, 1)
        assert result["press"] == [{"type": "note", "note": 60, "velocity": 100}]
        assert result["release"] == [{"type": "note", "note": 60, "velocity": 0}]
        assert result["color"] == "red"
